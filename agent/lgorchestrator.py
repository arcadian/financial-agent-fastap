...
def make_node(tool_call):
    print(f"got in make node")
    async def node(state):
        print(f"got inside node")
        tool_name = tool_call.get("tool_name")
        parameters = tool_call.get("parameters", {})
        tool_function = tool_registry[tool_name]["function"]

        result_data = await asyncio.to_thread(tool_function, **parameters)
        result = await _format_result(tool_name, tool_call, result_data)
        return {**state, str(tool_call["id"]): result}

    return node

def build_dag(task_spec):
    g = StateGraph(dict, merge_strategy="update")

    deps = []
    leaders = []

    for i, stage in enumerate(task_spec):
        for task in stage:
            print(f"creating node {task}")
            g.add_node(str(task["id"]), make_node(task))
            if i > 0:
                for t in task_spec[i - 1]:
                    g.add_edge(str(t["id"]), str(task["id"]))
                    deps.append(task["id"])
                    leaders.append(t["id"])

    print(f"got in make node")
    #entry_tasks = [t["id"] for t in task_spec if all(not isinstance(v, dict) for v in t["args"].values())]
    for stage in task_spec:
        for t in stage:
            if t["id"] not in deps:
                g.set_entry_point(str(t["id"]))

            if t["id"] not in leaders:
                g.add_edge(str(t["id"]), END)
...
async def run_financial_agent(query: str):
    try:
        ...
        all_results = await graph.ainvoke({})
        vals = []
        for i, val in enumerate(all_results.values()):
            vals.append(val)

        print(f"these are the values: {vals}")
        ...
        #             return {"tool_name": tool_name, "error": str(ie)}
        #
